...
def create_branch(new_branch, source_branch='main'):
    ...
    print(f'Fetching source branch from URL: {source_branch_url}')  # Debug print
    response = requests.get(source_branch_url, headers=headers)

    if response.status_code == 404:
        raise ValueError(f'Source branch "{source_branch}" not found: {response.json()}')
    elif not response.ok:
        print(f'Error response: {response.json()}')  # Debug print
