...
def check_api_connections(self) -> Dict[str, bool]:
    """Check connections to required APIs."""#-
    ...
    for api_name, api_config in api_configs.items():#+
        try:
            ...
            if "test_url" not in api_config:#-
                logger.warning(f"No test URL configured for API {api_name}")#-
                results[api_name] = None#-
                continue#-
#-
            # Get API key environment variable#-
            api_key_var = api_config.get(#-
                "api_key_var", f"{api_name.upper()}_API_KEY")#-
