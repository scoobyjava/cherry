...
class AIConfigPolicy:
    ...
    # Additional context rules that define how much of the file or project is analyzed#-
    # Context rules for different file types#+
    CONTEXT_RULES = {
        'deep_context': {
            'extensions': ['.py', '.js', '.ts', '.java'],#-
            'window_size': 2048,        # Number of tokens or lines considered#-
    ...
    SECURITY_PARAMS = {
        'model_whitelist': ['Tabnine-Prod-2025', 'Copilot-Enterprise', 'Codeium-Legacy'],#-
...
#-
        if settings_path.exists():#-
            try:#-
                ...
                logger.warning(#-
                    f"Could not parse {settings_path}. Creating new file.")#-
#-
        # Merge with existing settings#-
        existing_settings.update(vscode_config['settings'])#-
#-
...
#-
        ...
        existing_extensions = {"recommendations": []}#-
#-
        if extensions_path.exists():#-
            try:#-
                ...
                logger.warning(#-
                    f"Could not parse {extensions_path}. Creating new file.")#-
#-
        # Merge recommendations#-
        existing_recommendations = set(#-
            existing_extensions.get("recommendations", []))#-
        new_recommendations = set(#-
            vscode_config['extensions']['recommendations'])#-
#-
        existing_extensions["recommendations"] = list(#-
            existing_recommendations.union(new_recommendations))#-
#-
        with open(extensions_path, 'w') as f:#-
            json.dump(existing_extensions, f, indent=2)#-
#-
    def _apply_tabnine_config(self) -> None:#-
        """Apply Tabnine configuration"""#-
        tabnine_dir = Path(self.project_root) / ".tabnine"#-
        tabnine_dir.mkdir(exist_ok=True)#-
#-
        config_path = tabnine_dir / "config.json"#-
        existing_config = {}#-
#-
        if config_path.exists():#-
            try:#-
                ...
                logger.warning(#-
                    f"Could not parse {config_path}. Creating new file.")#-
#-
        # Apply our Tabnine settings#-
        tabnine_config = {#-
        ...
        existing_config.update(tabnine_config)#-
    ...
    def _apply_codeium_config(self) -> None:#-
        """Apply Codeium configuration"""#-
        codeium_dir = Path(self.project_root) / ".codeium"#-
    ...
    def get_tool_for_file(self, file_path: str, language: str = None) -> Dict[str, Any]:#+
        ...
        config_path = codeium_dir / "config.json"#-
        existing_config = {}#-
        Args:#+
...
#-
        ...
        # Merge with existing config#-
        existing_config.update(codeium_config)#-
#-
        with open(config_path, 'w') as f:#-
            json.dump(existing_config, f, indent=2)#-
#-
    def _apply_copilot_config(self) -> None:#-
        """Apply GitHub Copilot configuration"""#-
        # Copilot typically uses .vscode/settings.json, which is already handled#-
        # in _apply_vscode_config, but we could add additional Copilot-specific#-
        # configuration here if needed#-
        pass#-
#-
    @classmethod#-
    def generate_ide_config(cls, ide_type: str = 'vscode') -> Dict[str, Any]:#-
    def _detect_language(self, file_path: str) -> str:#+
        """
        Generates IDE-specific configuration from central policy#-
        Detect the programming language from a file path#+

        Args:
            ide_type: The type of IDE ('vscode', 'jetbrains', etc.)#-
            file_path: Path to the file#+

        Returns:
            Dictionary with IDE-specific configuration#-
            Detected language name#+
        """
        config_map = {#-
            'vscode': cls._build_vscode_config,#-
            'jetbrains': cls._build_jetbrains_config#-
            # Add more if needed (e.g., vim, emacs, etc.)#-
        ...
        return config_map[ide_type]()#-
        return extension_to_language.get(ext, 'unknown')#+

    @classmethod#-
    def _build_vscode_config(cls) -> Dict[str, Any]:#-
        """#-
    def apply_config(self) -> bool:#+
        """#+
        Apply the AI configuration to all supported tools in the project#+
