...
class AgentMemory:
    """Memory system for Cherry AI agents to store and retrieve information"""
    ...
    def store(self, key: str, value: Any) -> None:
        ...
        self._access_history[key] = {
            "stored": datetime.now(), "last_accessed": None}

    def retrieve(self, key: str) -> Any:
        """Retrieve a value and update access timestamp"""
        if key in self._storage:
            self._access_history[key]["last_accessed"] = datetime.now()
...
def share_memory_between_agents(source_agent, target_agent, content):
    ...
    try:
        ...
        conn.close()
        logger.info(
            f"Memory shared in PostgreSQL: {memory_id} from {source_agent} to {target_agent}")
    except Exception as e:
    ...
    try:
        ...
        if index_name not in pc.list_indexes():
            # adjust dimension based on your embedding size
        ...
        logger.info(
            f"Memory shared in Pinecone: {memory_id} from {source_agent} to {target_agent}")
    except Exception as e:
